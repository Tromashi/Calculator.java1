...
class Main {
    public static String[] calc(String inputString) {
        ...
        if (calc_inputString.length != 3) {
            Scanner inputString_incorrect_data_format = new Scanner(System.in);
            System.out.println("Неверный формат. Введите выражение, разделяя каждый символ пробелом");
            inputString = inputString_incorrect_data_format.nextLine();
...
class Romes extends Number {
    ...
    private String convert_result_to_Romes(int n, int remainder) {
        remainder = n % 10;
        if (remainder != 0) {
            try {
                return convert_result_to_Romes(n - remainder, 0) + roman_letters_9[remainder - 1];
            } catch (ArrayIndexOutOfBoundsException e) {
                sign = "-";
                return convert_result_to_Romes(n - remainder, 0) + roman_letters_9[(remainder + 1) * -1];
            }
        }
    ...
    public void div() {
        try {
            ...
            System.out.print("Проверьте правильность ввода. Вероятно введены и арабские и римские одновременно. ");
        }
